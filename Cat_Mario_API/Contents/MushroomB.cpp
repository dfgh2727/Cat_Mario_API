...
MushroomB::MushroomB()
{
...

	{
		...
		CollisionComponent->SetCollisionGroup(ECollisionGroup::GettingBig);
...
void MushroomB::BeginPlay()
{
	Super::BeginPlay();
}

void MushroomB::Tick(float _DeltaTime)
{
...
void MushroomB::Move(float _DeltaTime)
{
...
void MushroomB::Gravity(float _DeltaTime)
{
	if (false == IsGround)
	{
...
bool MushroomB::OnTheBlock(float _DeltaTime)
{
...
void MushroomB::TurnAround(FVector2D _MovePos)
{
	FVector2D MushroomPos = this->GetActorLocation();
...
bool MushroomB::GetOutOfBox(float _DeltaTime)
{
...
void MushroomB::RiseUp(float _DeltaTime)
{
